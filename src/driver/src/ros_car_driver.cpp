...
void turn_on_robot::Cmd_Vel_Callback(const geometry_msgs::Twist &twist_aux)
{
  ...
  //机器人x轴的目标线速度
  //★ 本车硬件: 负 linear.x = 前进, ROS 标准是正 = 前进, 此处取反适配硬件
  transition=0;
  transition = -twist_aux.linear.x*1000; //将浮点数放大一千倍并取反，简化传输
...
void turn_on_robot::Publish_Odom()
{
    ...
    //★ 本车硬件: STM32 上报负速度为前进, ROS 标准是正 = 前进, 此处取反
...
void turn_on_robot::Control()
{
  ...
  while(ros::ok())
  {
    ...
    {
      // ===== 修复: 收到数据后立即计算 dt, 并对异常 dt 限幅 =====
      // 原实现 dt 用上一轮 _Now 滞后一轮, 且丢帧时 _Last_Time 不更新
      // 导致下次用旧速度 × 大 dt 积分 -> 里程计漂移 -> 地图错位
      Sampling_Time = (_Now - _Last_Time).toSec();
      _Last_Time = _Now;

      // dt 限幅: 正常帧间隔 <0.2s, 超过 0.5s 视为丢帧/卡顿, 丢弃此帧积分
      if (Sampling_Time < 0.0)
        Sampling_Time = 0.0;
      else if (Sampling_Time > 0.5)
        Sampling_Time = 0.0;

      //★ 本车硬件: STM32 上报速度为负时实际前进, 位置积分取反修正
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<std::string>("odom_frame_id",    odom_frame_id,    "odom");      //The odometer topic corresponds to the parent TF coordinate //里程计话题对应父TF坐标(由 EKF 融合后输出 odom_combined)
